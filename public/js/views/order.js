...
export default async function order(main, { params, query, navigate }) {
  ...
  main.innerHTML = String(html`<div class="container section" style="max-width:860px">
    ...
    <div class="panel"><div class="section-head" style="margin:0"><div><span class="muted small">${t("orderNumber")}</span><h2 style="margin:0;font-family:var(--font-body);font-weight:700;letter-spacing:.04em">${o.order_no}</h2></div><span class="muted small">${date(o.created_at)}</span></div>
    ...
    <div class="panel"><h2>${t("itemsInOrder")}</h2>
      ${items.map((i) => html`<div class="line"><img src="${i.image}" alt="" width="72" height="96" loading="lazy"><div><b>${lang() === "bn" ? i.name_bn : i.name_en}</b><div class="meta">${i.size} · ${i.color} · ×${num(i.quantity)}</div></div><b>${money(i.line_total)}</b></div>`)}
